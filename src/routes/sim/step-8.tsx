			...
			box2Content={
				<>
					<p>
						...
						at a time (or the smallest sensible unit) instead of in big batches.
						Benefits: we see problems immediately, we hold less inventory, and
						when the market changes we have less WIP at risk — so we lose less
						and adapt faster. We switch to <strong>batch size one</strong> (and
						adjusted variance) so items move one by one. Now we're consistently
						making money <em>and</em> reacting to market changes: we listen to
						the customer, catch defects early, balance the line, and keep WIP
						low. That's Lean in action.
					</p>
					<p>
						Try the{" "}
						<Link
							to="/playground"
							className="text-sim-purple-border underline hover:no-underline"
						>
							playground
						</Link>{" "}
						next to experiment with all the levers after you've seen the full
						journey.
					</p>
				</>
			}
